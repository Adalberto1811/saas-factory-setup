...
export async function logSystemHealth(
    ...
    try {
        await sql`
            INSERT INTO system_health (endpoint, model, latency_ms, status_code, error_message)
            VALUES (${endpoint}, ${model}, ${latencyMs}, ${statusCode}, ${errorMessage || null})
        `;
    } catch (error) {
        console.error('[NeonLib] Error logging system health:', error);
    }
}
