...
| `PRP-002-creem-integration.md` | Integración de Creem.io (Pagos PSE) |
...
### 2026-01-17: Sincronización de Credenciales N8N
- **Error**: Credenciales hardcodeadas en proxy API y divergencia entre .mcp.json y .env.local.
- **Fix**: Centralizar credenciales en .env.local y refactorizar proxies para usar variables de entorno. Asegurar espejo en .mcp.json.
- **Aplicar en**: Todos los servicios que requieran proxies o configuración MCP.

### 2026-01-18: Error 502 en N8N (Social Publisher)
- **Error**: Workflow falla con 502 Bad Gateway por nodo "Postiz" obsoleto.
- **Fix**: Reemplazar por nodos HTTP Request estándar.

### 2026-01-19: Kling AI task_type Update
- **Error**: Error 404 en endpoint `/kling/image-to-video`.
- **Fix**: Usar endpoint `/task` con `task_type: "video_generation"`.

### [2026-01-26]: Cadena de Estabilidad AI
- **Error**: Cuotas agotadas en Gemini SDK y límites en OpenRouter Free.
- **Fix**: Implementar fallback escalonado (Gemini 1.5 -> Pro -> GPT) sobre OpenRouter Paid Tier.
- **Vercel**: Activar `runtime: 'edge'` para compatibilidad total con Web Request API.
- **Aplicar en**: Chat, MarketSyn y Landing Page.
