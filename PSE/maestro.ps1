...
function Show-MainMenu {
    ...
    Write-Host "  [I] SaaS Guardian (Brain/Insights)" -ForegroundColor Cyan
    Write-Host "  ----------------------------------------------------" -ForegroundColor DarkGray
...
function Invoke-Guardian {
    Write-Host ""
    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host "                SAAS GUARDIAN - V3 BRAIN" -ForegroundColor Cyan
    Write-Host "============================================================" -ForegroundColor Cyan
    Write-Host ""
    
    $MemoryPath = "$PSScriptRoot\.claude\memory_bank.json"
    if (-not (Test-Path $MemoryPath)) {
        Write-Host "   [!] No se encontro el Memory Bank." -ForegroundColor Yellow
        Write-Host "   Creando uno nuevo..." -ForegroundColor Gray
        $initialMemory = @{
            tribal_knowledge = @();
            learned_patterns = @();
    ...
    Write-Host "ESTADO DEL CEREBRO:" -ForegroundColor Green
    Write-Host "   Conocimientos tribales: $($Memory.tribal_knowledge.Count)" -ForegroundColor White
    Write-Host "   Patrones aprendidos:    $($Memory.learned_patterns.Count)" -ForegroundColor White
    Write-Host "   Insights detectados:    $($Memory.cross_project_insights.Count)" -ForegroundColor White
    Write-Host ""
    
    Write-Host "ULTIMOS PATRONES:" -ForegroundColor Cyan
    if ($Memory.learned_patterns.Count -gt 0) {
        $Memory.learned_patterns | Select-Object -Last 3 | ForEach-Object {
            Write-Host "   - $($_.pattern_name): $($_.insight)" -ForegroundColor Gray
        }
    } else {
        Write-Host "   (Vacio) Comienza a construir para generar patrones." -ForegroundColor DarkGray
    }
    Write-Host ""
